...
def normalize_from_onecall(one, units: str):
    ...
    return {"temp": temp, "tmin": tmin, "tmax": tmax, "condition": (desc or "").title(),
            "icon": icon, "sunrise": sunrise, "sunset": sunset, "humidity": humidity,
            "tz_offset": tz_off, "units": units}
...
def normalize_from_current(current, units: str, tz_off_guess=None):
    ...
    return {"temp": main.get("temp"), "tmin": main.get("temp_min"), "tmax": main.get("temp_max"),
            "condition": (w.get("description") or w.get("main") or "").title(),
            "icon": w.get("icon", ""), "sunrise": sysb.get("sunrise"), "sunset": sysb.get("sunset"),
            "humidity": main.get("humidity"), "tz_offset": off, "units": units}

def fetch_weather(cfg):
    api_key = cfg["api_key"]
    if not api_key: raise RuntimeError("OpenWeatherMap requires WEATHER_API_KEY")
    units = cfg["units"]
...
def normalize_temp_value(v, units: str):
    ...
    try:
        ...
        return None
    # If API was called with units=imperial or metric, values are already correct.
    if u.startswith("imp") or u.startswith("met"):
        return v
    # Fallback: treat as Kelvin if units are unknown/standard.
    return k_to_c(v)

def lerp(a,b,t): return a + (b-a)*t
def lerp_rgb(c1,c2,t): return (int(lerp(c1[0],c2[0],t)), int(lerp(c1[1],c2[1],t)), int(lerp(c1[2],c2[2],t)))
...
def temp_color(temp, units: str):
    ...
    if temp<=mid:
        t = max(0,min(1,(temp-lo)/(mid-lo))); return lerp_rgb(c1,c2,t)
    t = max(0,min(1,(temp-mid)/(hi-mid))); return lerp_rgb(c2,c3,t)
...
CLOCK_BDFS = ["10x20.bdf","9x18B.bdf","9x18.bdf"]
CURR_BDFS  = ["10x20.bdf","9x18B.bdf"]
...
def fmt_hhmm(ts, tz_off):
    ...
    try:
        ...
        if tz_off: dt = dt + timedelta(seconds=int(tz_off))
        s = dt.strftime("%I:%M")
...
# --- MLB PNG icons (tinted for visibility) ---
...
def _blit_tinted_rgba(canvas, img, x, y, tint_rgb):
    ...
    for j in range(h):
        yy = y + j
        if yy < 0 or yy >= 64: continue
        for i in range(w):
            xx = x + i
            if xx < 0 or xx >= 64: continue
            _,_,_,a = px[i,j]
            if a > 16:
...
def draw_moon_small(canvas, x, y):
    edge = graphics.Color(210,200,255)
    for a in range(0,360,10):
        x2 = x + int(3*math.cos(math.radians(a)))
        y2 = y + int(3*math.sin(math.radians(a)))
        canvas.SetPixel(x2, y2, edge.red,edge.green,edge.blue)
    for a in range(-60,60,10):
        x2 = x + int(3*math.cos(math.radians(a)))
        y2 = y + int(3*math.sin(math.radians(a)))
        canvas.SetPixel(x2+1, y2, 0,0,0)
...
def draw_frame(offscreen, fonts, wdata):
    ...
    graphics.DrawText(offscreen, f_small, drop_x+10, drop_y+9, graphics.Color(170,210,255), hum_txt)
    ...
    draw_moon_small(offscreen, (ss_text_x)-7, base_y-3)
...
def main():
    ...
    f_small, small_path = load_font(["7x13.bdf","6x12.bdf","6x10.bdf","5x8.bdf"])
    f_clock, clock_path = load_font(["10x20.bdf","9x18B.bdf","9x18.bdf"])
    f_curr,  curr_path  = load_font(["10x20.bdf","9x18B.bdf"])
    ...
    try:
        while True:
            ...
            frame_ctr += 1
            if frame_ctr % 1800 == 0:  # ~24 minutes at 0.8s per second worth of frames
                gc.collect()
