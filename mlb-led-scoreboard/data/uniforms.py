...
SPECIAL_UNIFORMS = {
    CITY_CONNECT: lambda uniformName: "City Connect" in uniformName
}
...
class Uniforms:
    def __init__(self, game_id):
        ...
        try:
            data = statsapi.get("game_uniforms", {"gamePks": game_id, "fields": API_FIELDS}).get("uniforms", [{}])[0]

            home_assets = data.get("home", {}).get("uniformAssets", [])
            away_assets = data.get("away", {}).get("uniformAssets", [])

            for (uniform, special_check) in SPECIAL_UNIFORMS.items():
                if not self.home_special and any(special_check(asset.get('uniformAssetText', '')) for asset in home_assets):
                    self.home_special = uniform
                if not self.away_special and any(special_check(asset.get('uniformAssetText', '')) for asset in away_assets):
