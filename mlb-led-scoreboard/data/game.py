...
class Game:
    ...
    def full_name(self, player):
    	...
    	name = self._current_data["gameData"]["players"][ID]["fullName"]
    	return name.replace(", ", ",")

    def boxscore_name(self, player):
    	ID = Game._format_id(player)
    	person = self._current_data["gameData"]["players"][ID]
    	name = person.get("boxscoreName", "")

    	if "," in name:  # Already has Last,F format
        	return name.replace(", ", ",")
    	else:
        	# Fallback to creating Last,F from full name
